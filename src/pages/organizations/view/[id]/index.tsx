...
import { deleteExpertById } from 'apiSdk/experts';
  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface>(
    ...
    () =>
      getOrganizationById(id, {
        relations: ['user', 'expert', 'service'],
      }),
  );

  const expertHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('expert', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Experts
                    </Text>
                    <NextLink passHref href={`/experts/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.expert?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/experts/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
